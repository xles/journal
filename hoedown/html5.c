...
#define USE_XHTML(opt) (opt->flags & HOEDOWN_HTML5_USE_XHTML)
...
hoedown_html5_is_tag(const uint8_t *tag_data, size_t tag_size, const char *tagname)
{
	...
	if (isspace(tag_data[i]) || tag_data[i] == '>')
		return closed ? HOEDOWN_HTML5_TAG_CLOSE : HOEDOWN_HTML5_TAG_OPEN;
...
{
	hoedown_html5_renderer_state *state = opaque;

	if (!link || !link->size)
		return 0;

	if ((state->flags & HOEDOWN_HTML5_SAFELINK) != 0 &&
...
rndr_blockcode(hoedown_buffer *ob, const hoedown_buffer *text, const hoedown_buffer *lang, void *opaque)
{
	...
	if (lang && lang->size) {
		size_t i, cls = 0;
		if (state->flags & HOEDOWN_HTML5_PRETTIFY) {
			HOEDOWN_BUFPUTSL(ob, "<figure><code class=\"prettyprint");
			cls++;
		} else {
			HOEDOWN_BUFPUTSL(ob, "<figure><code class=\"");
		}
		...
		HOEDOWN_BUFPUTSL(ob, "\">");
	} else if (state->flags & HOEDOWN_HTML5_PRETTIFY) {
		HOEDOWN_BUFPUTSL(ob, "<figure><code class=\"prettyprint\">");
	} else {
		HOEDOWN_BUFPUTSL(ob, "<figure><code>");
	}

	if (text)
		escape_html(ob, text->data, text->size);

	HOEDOWN_BUFPUTSL(ob, "</code></figure>\n");
}
...
rndr_header(hoedown_buffer *ob, const hoedown_buffer *text, int level, void *opaque)
{
	...
	if ((state->flags & HOEDOWN_HTML5_TOC) && (level <= state->toc_data.nesting_level))
...
rndr_link(hoedown_buffer *ob, const hoedown_buffer *link, const hoedown_buffer *title, const hoedown_buffer *content, void *opaque)
{
	...
	if (link != NULL && (state->flags & HOEDOWN_HTML5_SAFELINK) != 0 && !hoedown_autolink_is_safe(link->data, link->size))
		return 0;
...
rndr_paragraph(hoedown_buffer *ob, const hoedown_buffer *text, void *opaque)
{
	...
	if (state->flags & HOEDOWN_HTML5_HARD_WRAP) {
		size_t org;
		while (i < text->size) {
			org = i;
...
rndr_raw_html(hoedown_buffer *ob, const hoedown_buffer *text, void *opaque)
{
	...
	/* HTML5_ESCAPE overrides SKIP_HTML, SKIP_STYLE, SKIP_LINKS and SKIP_IMAGES
	* It doens't see if there are any valid tags, just escape all of them. */
	if((state->flags & HOEDOWN_HTML5_ESCAPE) != 0) {
		escape_html(ob, text->data, text->size);
		return 1;
	}

	if ((state->flags & HOEDOWN_HTML5_SKIP_HTML) != 0)
		return 1;

	if ((state->flags & HOEDOWN_HTML5_SKIP_STYLE) != 0 &&
		hoedown_html5_is_tag(text->data, text->size, "style"))
		return 1;

	if ((state->flags & HOEDOWN_HTML5_SKIP_LINKS) != 0 &&
		hoedown_html5_is_tag(text->data, text->size, "a"))
		return 1;

	if ((state->flags & HOEDOWN_HTML5_SKIP_IMAGES) != 0 &&
		hoedown_html5_is_tag(text->data, text->size, "img"))
		return 1;
...
hoedown_html5_toc_renderer_new(int nesting_level)
{
	...
	hoedown_html5_renderer_state *state;
	hoedown_renderer *renderer;

	/* Prepare the state pointer */
	state = malloc(sizeof(hoedown_html5_renderer_state));
	if (!state)
		return NULL;

	memset(state, 0x0, sizeof(hoedown_html5_renderer_state));

	if (nesting_level > 0) {
		state->flags |= HOEDOWN_HTML5_TOC;
...
hoedown_html5_renderer_new(unsigned int render_flags, int nesting_level)
{
	...
	if (render_flags & HOEDOWN_HTML5_SKIP_IMAGES)
		renderer->image = NULL;

	if (render_flags & HOEDOWN_HTML5_SKIP_LINKS) {
		renderer->link = NULL;
		renderer->autolink = NULL;
	}

	if (render_flags & HOEDOWN_HTML5_SKIP_HTML || render_flags & HOEDOWN_HTML5_ESCAPE)
